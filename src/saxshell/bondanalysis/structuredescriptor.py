...
class StructureDescriptor:
    """Reads PDB/XYZ structures, aligns by anchor atoms, computes
    descriptors, geometric centers, coordination numbers, and provides
    visualization.
    ...
    def read_structures(self) -> List[str]:
        ...
        if not files:
            raise ValueError(
                f"No .pdb/.xyz files found in {self.input_folder!r}"
            )
        for fn in files:
    ...
    def _parse_pdb_line(self, line: str) -> AtomPlus:
        ...
        element = (
            line[76:78].strip() or re.sub(r"[^A-Za-z]", "", atom_name)[:2]
        )
        return AtomPlus(
            atom_id, atom_name, residue_name, residue_number, x, y, z, element
        )
    ...
    def _read_xyz(self, filepath: str) -> List[AtomPlus]:
        ...
        try:
            ...
            raise ValueError(
                f"XYZ file must start with atom count: {filepath}"
            )
    ...
    def align_structures(self) -> None:
        """Align all structures to the first (reference) via anchor
        atoms."""
    ...
    def _align_pair(
        ...
        ref_pts = np.array(
            [[a.x, a.y, a.z] for a in self.get_anchor_atoms(ref)]
        )
        tgt_pts = np.array(
            [[a.x, a.y, a.z] for a in self.get_anchor_atoms(tgt)]
        )
        ...
        if n == 1:
            ...
            axis = np.cross(
                v_t / np.linalg.norm(v_t), v_r / np.linalg.norm(v_r)
            )
            if np.linalg.norm(axis) < 1e-6:
                ...
                ang = np.arccos(
                    np.clip(
                        np.dot(v_t, v_r)
                        / (np.linalg.norm(v_t) * np.linalg.norm(v_r)),
                        -1,
                        1,
                    )
                )
    ...
    def compute_histogram_descriptor(
        ...
        for struct in self.structures:
            ...
            hist, _ = np.histogram(
                vals, bins=bins, range=(lo, hi), density=True
            )
            desc.append(hist)
    ...
    def compute_element_centers(self) -> List[Dict[str, np.ndarray]]:
        ...
        **relative** to the anchor‐atom center.  Also builds:
        ...
        for struct in self.structures:
            ...
            if not anchors:
                raise ValueError(
                    "No anchor atoms found for computing centers."
                )
            ...
            for el in set(a.element for a in struct):
                coords = np.array(
                    [[a.x, a.y, a.z] for a in struct if a.element == el]
                )
    ...
    def match_atoms_to_reference(
        ...
        for i, a in enumerate(ref):
            rd.setdefault(a.element, []).append((i, np.array([a.x, a.y, a.z])))
        for i, a in enumerate(tgt):
            td.setdefault(a.element, []).append((i, np.array([a.x, a.y, a.z])))
        m = {}
    ...
    def match_all_atoms(self) -> List[Dict[int, int]]:
        ref = self.structures[0]
        return [
            self.match_atoms_to_reference(ref, tgt)
            for tgt in self.structures[1:]
        ]

    def compute_coordination_numbers(
        self,
    ) -> List[Dict[Tuple[str, str], List[int]]]:
        """For each structure, compute coordination numbers for each
        (absorber, neighbor) pair based on self.coord_cutoffs, storing
        raw counts and sorted sets."""
            ...
            for pair, cutoff in self.coord_cutoffs.items():
                ...
                for a in absorbers:
                    ...
                    nbrs = [
                        ...
                        for b in struct
                        if b.element == nbr_el
                        and not (abs_el == nbr_el and b.atom_id == a.atom_id)
                    ]
                    dists = [
                        np.linalg.norm(
    ...
    def plot_coordination_histograms(
        ...
        """For each atom-atom pair, plot a bar chart of coordination-
        number sets frequency."""
    ...
    def plot_element_center_histograms(
        ...
        """For each non-anchor element type, plot a histogram of the
        distance between that element’s geometric center and the
        anchor‐atom geometric center across all structures."""
        if not self.element_centers:
            raise RuntimeError(
                "You must run() first so that element_centers are available."
            )
        ...
        print(
            f"[sanity] anchor '{anchor}' centroid dists: min={anchor_dists.min():.3e}, "
    ...
    def plot_alignment(self) -> None:
        ...
        # plot reference
        ref_anc = np.array(
            [[a.x, a.y, a.z] for a in ref if a.element == self.anchor_atom]
        )
        ref_oth = np.array(
            [[a.x, a.y, a.z] for a in ref if a.element != self.anchor_atom]
        )
        ref_anchor_scatter = ax.scatter(
            *ref_anc.T, c="blue", marker="^", s=80, label="Ref Anchor"
        )
        ref_other_scatter = None
        if ref_oth.size:
            ref_other_scatter = ax.scatter(
                *ref_oth.T, c="blue", marker="o", s=80, label="Ref Other"
            )
        ...
        def on_pick(event):
            ...
            if len(measure_pts) == 2:
                ...
                (meas_line,) = ax.plot(
                    [p1[0], p2[0]], [p1[1], p2[1]], [p1[2], p2[2]], "k-", lw=2
                )
                mid = (p1 + p2) / 2
        ...
        def update(idx):
            ...
            clear_meas()
            if target_anchor_scatter:
                target_anchor_scatter.remove()
            if target_other_scatter:
                target_other_scatter.remove()
            for ln in lines:
                ln.remove()
            lines.clear()
            ...
            offset = (
                (np.mean(anc_ref, axis=0) - np.mean(anc_tgt, axis=0))
                if anc_ref
                else np.zeros(3)
            )
            tgt_arr += offset
            ...
            ta_arr = (
                (np.array(anc_tgt) + offset) if anc_tgt else np.empty((0, 3))
            )
            to_arr = (
                (np.array(oth_tgt) + offset) if oth_tgt else np.empty((0, 3))
            )
            target_anchor_scatter = ax.scatter(
                *ta_arr.T,
                c="red",
                marker="^",
                s=80,
            ...
            target_other_scatter = ax.scatter(
                *to_arr.T,
                c="red",
                marker="o",
                s=80,
    ...
    def run(
        ...
        plt.close("all")
        if verbose:
            print("[1] Reading structures…")
        self.read_structures()
        if verbose:
            print("[2] Aligning…")
        self.align_structures()
        if verbose:
            print("[3] Computing element centers…")
        self.compute_element_centers()

        if self.coord_cutoffs:
            if verbose:
                print("[4] Computing coordination numbers…")
        ...
        if verbose:
            print(
                "[6] Done. Descriptors, centers, and coordination numbers available."
            )
        return results
