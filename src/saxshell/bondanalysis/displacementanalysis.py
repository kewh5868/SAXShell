...
class DisplacementAnalysis:
    """
    Analyzes cluster folders for bond/angle (distribution/angle) files,
    labels each as 'Bond Pair' or 'Angle Triplet', and loads the relevant data.
    """
    ...
    def _find_structure_folders(self):
        return [
            ...
            if os.path.isdir(os.path.join(self.ana_dir, entry)) and not entry.startswith('representative_')
        ]
    ...
    def _scan_all_structures_and_load_data(self):
        ...
        for struct_path in self.structure_folders:
            ...
            for fname in os.listdir(outdist_path):
                ...
                if fname.endswith('_distribution.csv'):
                    m = re.match(r"^([A-Za-z0-9]+_[A-Za-z0-9]+)_distribution\.csv$", fname)
                    if m:
                        pair = m.group(1)
                        label = (pair, 'Bond Pair')
                        bond_pairs[pair] = fpath
                        # Load third column values
                        self.data[(struct_name, label)] = self._load_third_column_from_csv(fpath)
                # Angle Triplets (_angles.csv)
                elif fname.endswith('_angles.csv'):
                    m = re.match(r"^([A-Za-z0-9]+_[A-Za-z0-9]+_[A-Za-z0-9]+)_angles\.csv$", fname)
                    if m:
                        triplet = m.group(1)
        ...
        self.unique_bond_pairs = set().union(*all_bond_pairs) if all_bond_pairs else set()
        self.unique_angle_triplets = set().union(*all_angle_triplets) if all_angle_triplets else set()
        self.shared_bond_pairs = set.intersection(*all_bond_pairs) if all_bond_pairs else set()
        self.shared_angle_triplets = set.intersection(*all_angle_triplets) if all_angle_triplets else set()

    def _load_third_column_from_csv(self, filepath):
        """Extracts the third column (as floats) from a csv, skipping header row."""
        col = []
        try:
    ...
    def merge_shared_distributions_and_angles(self):
        """
        For each shared bond pair or angle triplet, create a merged CSV
        containing all structure values, output to the representative_structures folder.
        ...
        for cand in ['representative_structures', 'representative_clusters']:
            ...
            if os.path.isdir(rep_path):
                ...
                break
        if not rep_folder:
            raise FileNotFoundError("No representative_structures or representative_clusters folder found.")
    ...
    def print_summary(self):
        ...
        print(f"Shared Bond Pairs ({len(self.shared_bond_pairs)}): {sorted(self.shared_bond_pairs)}")
        print(f"\nUnique Angle Triplets ({len(self.unique_angle_triplets)}): {sorted(self.unique_angle_triplets)}")
        print(f"Shared Angle Triplets ({len(self.shared_angle_triplets)}): {sorted(self.shared_angle_triplets)}")
    ...
    def get_max_bond_length_from_csv(self, csv_filepath: str, column_index: int = 2) -> float:
        """
        Reads a merged CSV of bond lengths or angles and returns the maximum value in the specified column.
        """
        import csv
        values = []
        with open(csv_filepath, 'r') as f:
            ...
            if column_index >= len(headers):
                raise IndexError(f"Column index {column_index} out of range for headers: {headers}")
            ...
            for row in reader:
                try:
                    ...
                    continue
        if not values:
            raise ValueError(f"No numeric values found in column '{key}' of {csv_filepath}")
        return max(values)
    ...
    def extract_bond_lengths_from_xyz_folder(
        ...
        """
        Extracts bond lengths between two element types from each XYZ file in a folder.
        If no cutoff is provided, it will be computed from the merged CSV.
        Returns a dict mapping filenames to numpy arrays of distances.
        """
        import os
        if cutoff is None:
            if merged_csv_filepath:
                cutoff = self.get_max_bond_length_from_csv(merged_csv_filepath, column_index)
            else:
                raise ValueError("Either 'cutoff' or 'merged_csv_filepath' must be provided")
    ...
    def compute_bond_lengths_for_pair(
        ...
        """
        Compute bond lengths for a selected bond pair from XYZ files in each structure folder.
        ...
        if cutoff is None and merged_csv_filename is None:
            merged_csv_filename = f"{element_pair[0]}_{element_pair[1]}_merged.csv"
        ...
        merged_csv_path = os.path.join(rep_folder, merged_csv_filename) if merged_csv_filename else None

        # Compute cutoff from merged CSV if not specified
        if cutoff is None and merged_csv_path and os.path.exists(merged_csv_path):
            cutoff = self.get_max_bond_length_from_csv(merged_csv_path, column_index)
        elif cutoff is None:
    ...
    def get_representative_motif_values(self, bond_or_angle, type_label, cutoff=None):
        """
        Finds all motif representative XYZ files for the given bond pair or angle triplet.
        ...
        for cand in ['representative_structures', 'representative_clusters']:
            ...
            if os.path.isdir(test):
                ...
                break
        if not rep_folder:
            print("[WARN] No representative_structures or representative_clusters folder found.")
            return {}

        motif_vals = {}
        # Loop all structures
        for struct in self.structure_distribution_map:
            outdist_path = os.path.join(self.ana_dir, struct, "output_distributions")
            if not os.path.isdir(outdist_path):
                continue
            ...
            for pdbfile in os.listdir(outdist_path):
                ...
                if type_label == "Bond Pair":
                    ...
                    for ci in (i for i, e in enumerate(types) if e == ce):
                        nbrs = tree.query_ball_point(coords[ci], r=max(cut1, cut2))
                        for i in nbrs:
                            if types[i] != n1:
                                continue
    ...
    def load_md_prior_weights(self, filepath=None):
        """
        Loads the md_prior_weights.json file and maps (structure, motif) to its count, weight, and representative pdb.
        If filepath is not provided, defaults to md_prior_weights.json in the analysis directory.
        Result is stored in self.md_prior_weights_map as:
        {(structure, motif): {"count": int, "weight": float, "representative": str}}
        """
        if filepath is None:
            ...
            for structure, motif_dict in data.get("structures", {}).items():
                for motif, motif_info in motif_dict.items():
                    mapping[(structure, motif)] = {
                        ...
                        "representative": motif_info.get("representative", None)
                    }
            self.md_prior_weights_map = mapping
            print(f"[INFO] Loaded md_prior_weights.json with {len(mapping)} (structure, motif) entries.")
        except Exception as e:
            print(f"[ERROR] Failed to load md_prior_weights.json: {e}")
            self.md_prior_weights_map = {}

    def plot_merged_distribution(self, bond_or_angle, type_label='Bond Pair', bins=100, show=True, ax=None, mode="histogram_positions"):
        """
        Plot the merged distribution for a specified bond pair or angle triplet.
        ...
        if not hasattr(self, 'merged_data') or merged_filename not in self.merged_data:
            raise ValueError(f"Run merge_shared_distributions_and_angles first, or no merged data for {bond_or_angle}.")
        ...
        motif_vals = self.get_representative_motif_values(bond_or_angle, type_label)
        ...
        if mode == "histogram":
            ...
            rep_positions = {ml: np.mean(vals) for ml, vals in motif_vals.items() if vals}
            # Gather motif counts
            counts = {}
            total_counts = 0
            for motif_label in rep_positions:
                ...
                cnt = self.md_prior_weights_map.get((struct, motif_key), {}).get('count', 0) or 0
                counts[motif_label] = cnt
                total_counts += cnt
            if total_counts <= 0:
                print(f"[WARN] No motif counts available for weighted mode; skipping motif lines.")
            else:
                # Get max bin height from histogram
                n, bins_, patches = ax.hist(values, bins=bins, alpha=0)  # Just to get heights; alpha=0 hides them
                hist_max = np.max(n) if len(n) else 1
                # Calculate unscaled motif heights
                motif_heights = {ml: (counts[ml] / total_counts) * total_hist_count for ml in rep_positions}
                max_motif_height = max(motif_heights.values()) if motif_heights else 1
                # Scaling factor to set max motif line to histogram max
                scale = hist_max / max_motif_height if max_motif_height > 0 else 1
                ...
                for motif_label, pos in rep_positions.items():
                    ...
                    ax.vlines(pos, 0, height, color='black', lw=1, linestyle='-', alpha=1.0)
        else:
            raise ValueError("mode must be 'histogram', 'histogram_positions' or 'histogram_weighted'")

        ax.set_xlabel('Bond Length (Å)' if type_label == 'Bond Pair' else 'Angle (deg)')
        ax.set_ylabel('Count')
    ...
    def fit_histogram_with_weighted_gaussians_tk(
        ...
        """
        Fits the merged histogram with a sum of Gaussians (see main method), but returns a matplotlib Figure
        for embedding into a TK window if show=False, or displays plot if show=True.
        """
        import matplotlib
        import matplotlib.pyplot as plt
        ...
        for motif_label, vals in motif_vals.items():
            if '_motif_' in motif_label:
                ...
                struct, motif_key = motif_label, None
            count = self.md_prior_weights_map.get((struct, motif_key), {}).get('count', 0) or 0
            for v in vals:
        ...
        if min_val is not None: values = values[values >= min_val]
        if max_val is not None: values = values[values <= max_val]
        counts, edges = np.histogram(values, bins=bins, range=(min_val, max_val))
        ...
        try:
            popt, pcov = curve_fit(
                ...
                bin_centers, counts, p0=[sigma0], bounds=([0.001],[10])
            )
        except Exception as e:
            print("Fit failed:", e)
            return None
        ...
        ax.hist(values, bins=bins, range=(min_val, max_val), alpha=0.5, color='tab:blue', label='Merged histogram')
        xfit = np.linspace(bin_centers[0], bin_centers[-1], 1000)
        ax.plot(xfit, sum_of_gaussians(xfit, sigma_fit), 'k-', label='Weighted Gaussian sum fit')
        ...
        if components == "lines":
            ...
            for i, (mu, area) in enumerate(zip(centers, motif_areas)):
                ax.plot(xfit, gauss(xfit, mu, sigma_fit, area), '--', lw=1.5, alpha=0.7, label=f'G{i+1}')
        # (components == "none": do nothing extra)

        ax.set_xlabel('Bond Length (Å)' if type_label == 'Bond Pair' else 'Angle (deg)')
        ax.set_ylabel('Count')
        ax.set_title(f'Fit: {bond_or_angle}, FWHM={fwhm_fit:.2f}, sigma² (XAFS Input)={sigma2_fit:.4f}')
        ax.legend()
        fig.tight_layout()
    ...
    def fit_histogram_with_weighted_gaussians(
        ...
        """
        Fits the merged histogram with a sum of Gaussians centered at every representative value from all motifs
        within min_val and max_val, each weighted by motif counts from md_prior_weights.
        ...
        if show:
            ...
            plt.hist(values, bins=bins, range=(min_val, max_val), alpha=0.5, color='tab:blue', label='Merged histogram')
            xfit = np.linspace(bin_centers[0], bin_centers[-1], 1000)
            plt.plot(xfit, sum_of_gaussians(xfit, sigma_fit), 'k-', label='Weighted Gaussian sum fit')
            ...
            if components == "lines":
                ...
                for i, (mu, area) in enumerate(zip(centers, motif_areas)):
                    plt.plot(xfit, gauss(xfit, mu, sigma_fit, area), '--', lw=1.5, alpha=0.7, label=f'G{i+1}')
            # (components == "none": do nothing extra)

            plt.xlabel('Bond Length (Å)' if type_label == 'Bond Pair' else 'Angle (deg)')
            plt.ylabel('Count')
            # plt.legend()
            plt.title(f'Fit: {bond_or_angle}, FWHM={fwhm_fit:.2f}, sigma² (XAFS Input)={sigma2_fit:.4f}')
            plt.tight_layout()
            plt.show()
    ...
    def save_weighted_gaussian_fit_report(
        ...
        """
        Save fit summary, histogram, components, and advanced fit statistics.
        Computes:
        ...
        total_fit = sum(gauss(bin_centers, mu, sigma, area)
                        for mu, area in zip(centers, motif_areas))
        components = [gauss(bin_centers, mu, sigma, area)
                    for mu, area in zip(centers, motif_areas)]
        ...
        with open(save_path, 'w') as f:
            ...
            f.write(f"# Fit range: {min(bin_centers):.6f} to {max(bin_centers):.6f}\n")
            ...
            for idx in range(n):
                row = [f"{bin_centers[idx]:.6f}", f"{hist[idx]:.6f}", f"{total_fit[idx]:.6f}"]
